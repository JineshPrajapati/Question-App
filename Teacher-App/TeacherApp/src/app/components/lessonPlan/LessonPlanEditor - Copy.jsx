...
export const LessonPlanEditor = ({ lessonFormat,
    lessonData
}) => {
                                    ...
                                    switch (lessonFormat) {
                                            ...
                                            return Array.from(
                                                new Map(
                                                    parsedContent.map((item,index) => [
                                                        item.Grading_Period,
                                                        {
                                                            Period: "Grading Period " + (index + 1) + " : " + item.Grading_Period,
                                                            Index: (index + 1),
                                            ...
                                            return Array.from(
                                                new Map(
                                                    parsedContent.map((item, index) => [
                                                        ...
                                                        {
                                                            Period: dayjs(item.Start_Date).format("MMMM YYYY"),
                                                            Index: (index + 1),
                                                            Start_Date: item.Start_Date,
                                                            End_Date: item.End_Date,
                                                            Domain_Title: item.Domain_Title,
                                        ...
                                        case "Daily":
                                            ...
                                            return parsedContent.map((item,index) => ({
                                                ...
                                                Materials_Resources_Needed: item.Materials_Resources_Needed || [],
                                                Anticipatory_Set: item.Anticipatory_Set || [],
                                                Objective_Purpose: item.Objective_Purpose || [],
                                                Input: item.Input || [],
                                                Model: item.Model || [],
                                                Check_for_Understanding: item.Check_for_Understanding || [],
        ...
        <div>
            ...
            <Box display="flex" justifyContent="flex-end" alignItems="center" mb={2}>
                ...
                <FormControlLabel
                    ...
                    label="Preview Mode"
                    sx={{ mt: 2 }} // remove extra margin to align properly
                />

                <Button sx={{ mt: 2 }} color="primary" variant="outline" onClick={handleSaveLessonPlan} className="flex items-center gap-2">
            ...
            {previewMode && (
                <MemoizedLessonPlanExport lessonFormat={lessonFormat} lessonPlans={lessonPlans} lessonData={lessonData} />
            )}

            {!previewMode && lessonPlans && lessonPlans.map((plan, index) => (
                <Collapsible key={index} defaultOpen={false} className="animate-fade-in">
                    <CollapsibleTrigger className="flex items-center justify-between w-full rounded-lg transition-colors"
                        style={{
                       ...
                       {plan.Period} 
                        ...
                        <ChevronDown
                            className="h-4 w-4 transition-transform duration-300 data-[state=closed]:rotate-180"
                        />
                    </CollapsibleTrigger>
                    ...
                    <CardContent>
                        <Grid container spacing={2}>
                                    ...
                                    <Grid container spacing={2}>
                                        ...
                                        <Grid item xs={12} sm={4}>
                                            <DatePicker
                                                label="Start Date"
                                                value={plan.Start_Date ? dayjs(plan.Start_Date) : null}
                                                onChange={(date) => {
                                                    ...
                                                    if (plan.End_Date) {
                                                        const weeks = Math.ceil(
                                                            (new Date(plan.End_Date) - new Date(formattedDate)) /
                                                            (1000 * 60 * 60 * 24 * 7)
                                                        );
                                                        handleChange(index, "Weeks_Consider", weeks > 0 ? weeks : 0);
                                                    }
                                                }}
                                        ...
                                        <Grid item xs={12} sm={4}>
                                                ...
                                                onChange={(date) => {
                                                    ...
                                                    if (plan.Start_Date) {
                                                        const weeks = Math.ceil(
                                                            (new Date(formattedDate) - new Date(plan.Start_Date)) /
                                                            (1000 * 60 * 60 * 24 * 7)
                                                        );
                                                        handleChange(index, "Weeks_Consider", weeks > 0 ? weeks : 0);
                                                    }
                                    ...
                                    <TextField
                                        ...
                                        onChange={(e) => handleChange(index, "Domain_Title", e.target.value)}
                                    />                                   
                        </Grid>
                        ...
                        <Divider sx={{ my: 2 }} />
                                    <Box display="flex" alignItems="center" justifyContent="space-between">
                                        ...
                                        <Button className="w-full h-[30px] text-white bg-primary hover:opacity-90 transition-smooth"
                                            startIcon={<Add />}
                                ...
                                <DragDropContext onDragEnd={(result) => onDragEnd(result, index, "Topics")}>
                                    <Droppable droppableId={`topics-${index}`}>
                                        {(provided) => (
                                            <Box ref={provided.innerRef} {...provided.droppableProps}>
                                                {plan.Topics.map((topic, tIndex) => (
                                                    <Draggable key={`topic-${tIndex}`} draggableId={`topic-${tIndex}`} index={tIndex}>
                                                        {(provided) => (
                                                            <Box
                                                            ...
                                                            >
                                                                <TextField
                                                                    ...
                                                                    size="small"
                                                                    sx={{ minHeight: 40 }}
                                                                    onChange={(e) => handleArrayChange(index, "Topics", tIndex, e.target.value)}
                                                                />
                                                                <IconButton color="error" onClick={() => removeArrayItem(index, "Topics", tIndex)}>
                                                                    <Delete />
                                                                </IconButton>
                                                            </Box>
                                                        )}
                        ...
                        <Divider sx={{ my: 2 }} />
                                ...
                                <DragDropContext onDragEnd={(result) => onDragEnd(result, index, "Standards")}>
                                    <Droppable droppableId={`standards-${index}`}>
                                        {(provided) => (
                                            <Box ref={provided.innerRef} {...provided.droppableProps}>
                                                {plan.Standards.map((std, sIndex) => (
                                                    <Draggable key={`standard-${sIndex}`} draggableId={`standard-${sIndex}`} index={sIndex}>
                                                        {(provided) => (
                                                            <Box
                                                                ref={provided.innerRef}
                                                                {...provided.draggableProps}
                                                            ...
                                                            >
                                                                <TextField
                                                                    ...
                                                                    size="small"
                                                                    sx={{ minHeight: 40 }}
                                                                    onChange={(e) => handleArrayChange(index, "Standards", sIndex, e.target.value)}
                                                                />
                                                                <IconButton color="error" onClick={() => removeArrayItem(index, "Standards", sIndex)}>
                                                                    <Delete />
                                                                </IconButton>
                                                            </Box>
                                                        )}
                               ...
                               <Divider sx={{ my: 2 }} />
                                    ...
                                    <DragDropContext onDragEnd={(result) => onDragEnd(result, index, "Activities")}>
                                        <Droppable droppableId={`activities-${index}`}>
                                            {(provided) => (
                                                <Box ref={provided.innerRef} {...provided.droppableProps}>
                                                {plan.Activities.map((std, sIndex) => (
                                                        <Draggable key={`activity-${sIndex}`} draggableId={`activity-${sIndex}`} index={sIndex}>
                                                            {(provided) => (
                                                                <Box
                                                                    ref={provided.innerRef}
                                                                    {...provided.draggableProps}
                                                                ...
                                                                >
                                                                    <TextField
                                                                        ...
                                                                        size="small"
                                                                        sx={{ minHeight: 40 }}
                                                                    onChange={(e) => handleArrayChange(index, "Activities", sIndex, e.target.value)}
                                                                    />
                                                                <IconButton color="error" onClick={() => removeArrayItem(index, "Activities", sIndex)}>
                                                                        <Delete />
                                                                    </IconButton>
                                                                </Box>
                                                            )}
                                    ...
                                    <Box display="flex" alignItems="center" justifyContent="space-between">
                                        ...
                                        >
                                            ...
                                            <DragDropContext onDragEnd={(result) => onDragEnd(result, index, "Assessments")}>
                                                <Droppable droppableId={`assessments-${index}`}>
                                                    {(provided) => (
                                                        <Box ref={provided.innerRef} {...provided.droppableProps}>
                                                            {plan.Activities.map((std, sIndex) => (
                                                                <Draggable key={`assessment-${sIndex}`} draggableId={`assessment-${sIndex}`} index={sIndex}>
                                                                    {(provided) => (
                                                                        <Box
                                                                            ref={provided.innerRef}
                                                                            {...provided.draggableProps}
                                                                        ...
                                                                        >
                                                                            <TextField
                                                                                ...
                                                                                size="small"
                                                                                sx={{ minHeight: 40 }}
                                                                                onChange={(e) => handleArrayChange(index, "Assessments", sIndex, e.target.value)}
                                                                            />
                                                                            <IconButton color="error" onClick={() => removeArrayItem(index, "Assessments", sIndex)}>
                                                                                <Delete />
                                                                            </IconButton>
                                                                        </Box>
                                                                    )}
