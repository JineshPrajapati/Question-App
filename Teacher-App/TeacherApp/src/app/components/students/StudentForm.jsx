...
export const StudentForm = ({
  ...
  const { student: currentUser } = useContext(AuthContext);
  ...
  return (
    <BaseForm
      ...
      onSubmit={(values) => {
        const payload = {
          ...values,
          studentTypeId: parseInt(values.type),
          isActive: true,
          createdIdentityBy: currentUser.studentId,
      ...
      onSuccess={onSuccess}
      onClose={onClose}
      submitButtonText={student ? "Update Student" : "Create Student"}
    >
      {(formik) => (
        <>
          ...
          {/* {showTeacherFields && ( */}
          <>{/* ProfilePicturePath is remaining     */}</>
          {/* )} */}
        </>
      )}
    </BaseForm>
